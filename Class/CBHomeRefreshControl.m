...
NSString *const startHomePointKey = @"startPoints";
NSString *const endHomePointKey = @"endPoints";
NSString *const xHomeKey = @"x";
NSString *const yHomeKey = @"y";

@interface CBHomeRefreshControl () <UIScrollViewDelegate>
...
@implementation CBHomeRefreshControl
...
{
    return [CBHomeRefreshControl attachToScrollView:scrollView
                                                   target:target
...
{
    CBHomeRefreshControl *refreshControl = [[CBHomeRefreshControl alloc] init];
    ...
    NSArray *startPoints = [rootDictionary objectForKey:startHomePointKey];
    NSArray *endPoints = [rootDictionary objectForKey:endHomePointKey];
    ...
    for (int i=0; i<startPoints.count; i++) {
        ...
        HomeItem *barItem = [[HomeItem alloc] initWithFrame:refreshControl.frame startPoint:startPoint endPoint:endPoint color:color lineWidth:lineWidth];
        barItem.tag = i;
    ...
    for (HomeItem *barItem in refreshControl.barItems) {
...
{
    if (self.originalTopContentInset == 0) self.originalTopContentInset = 0;
...
{
    for (HomeItem *barItem in self.barItems) {
...
- (void)startLoadingAnimation
{
    if (self.reverseLoadingAnimation) {
        ...
        for (int i= count-1; i>=0; i--) {
            HomeItem *barItem = [self.barItems objectAtIndex:i];
...
- (void)barItemAnimation:(HomeItem*)barItem
{
    if (self.state == CBStoreHouseRefreshControlStateRefreshing) {
        barItem.alpha = 1;
