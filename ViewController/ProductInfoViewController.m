...
@interface ProductInfoViewController ()<UITableViewDataSource,UITableViewDelegate,UIWebViewDelegate,PicCommentSpecial>
...
- (void)viewDidLoad {
    ...
    [self leftButton];
    [self rightButton];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(limitInfoDownloadFinish) name:[NSString stringWithFormat:PRODUCT_INFO,self.productId] object:nil];
    [[LVDownLoadManager sharedDownLoadManager]downLoadWithUrl:[NSString stringWithFormat:PRODUCT_INFO,self.productId] and:6];
    NSLog(@"%@",[NSString stringWithFormat:PRODUCT_INFO,self.productId]);
...
{
    _dataArray=[[LVDownLoadManager sharedDownLoadManager]getDownLoadDataForKey:[NSString stringWithFormat:PRODUCT_INFO,self.productId]];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(limitCommentDownloadFinish) name:[NSString stringWithFormat:PRODUCT_COMMENT,self.productId] object:nil];
    
    [[LVDownLoadManager sharedDownLoadManager]downLoadWithUrl:[NSString stringWithFormat:PRODUCT_COMMENT,self.productId] and:4];
    
    NSLog(@"comment=%@",[NSString stringWithFormat:PRODUCT_COMMENT,self.productId]);
    
    [self hudHide];
    [self createTop:[[_dataArray firstObject] objectForKey:@"clientImageBaseVos"]];
...
{
    _commentArray=[[LVDownLoadManager sharedDownLoadManager]getDownLoadDataForKey:[NSString stringWithFormat:PRODUCT_COMMENT,self.productId]];
}
...
-(void)createRest
{
    ...
    if ([[[_dataArray firstObject] objectForKey:@"categoryName"] length]) {
        cellAddress.text=[NSString stringWithFormat:@"%@ | %@",[[_dataArray firstObject] objectForKey:@"categoryName"],[[_dataArray firstObject] objectForKey:@"fromDest"]];
    ...
    NSString *stockCountstring=[NSString stringWithFormat:@"￥%@起/人",[NSString stringWithFormat:@"%d",[[[_dataArray firstObject] objectForKey:@"sellPrice"] intValue]]];
    ...
    [stockAttribute addAttributes:stockColor range:NSMakeRange(0,stockCountstring.length-3)];
    
    cellSellPriceYuan.attributedText=stockAttribute;
    cellSellPriceYuan.frame=CGRectMake(WIDTH,cellAddress.frame.origin.y, 100, 20);
    [cellSellPriceYuan sizeToFit];
    CGRect sellRect=cellSellPriceYuan.frame;
    sellRect.origin.x=self.view.bounds.size.width-sellRect.origin.x-sellRect.size.width;
    cellSellPriceYuan.frame=sellRect;
    ...
    _restHeight=separateLine.frame.origin.y+separateLine.frame.size.height;
    ...
    if ([[[_dataArray firstObject] objectForKey:@"clientRouteProductTagItemList"] count]) {
        ...
        NSArray * secondTagItems=[[_dataArray firstObject]objectForKey:@"clientRouteProductTagItemList"];
        
        for (int i=0; i<secondTagItems.count;i++) {
            NSDictionary *dict=[secondTagItems objectAtIndex:i];
            
            if ([[dict objectForKey:@"tagType"]isEqualToString:@"promotion"]) {
    ...
    for (NSDictionary *dict in [[_dataArray firstObject] objectForKey:@"clientProdProductPropBaseVos"]) {
        if ([[dict objectForKey:@"code"]isEqualToString:@"recommend"]) {
            ...
            desc.numberOfLines=0;
            desc.tag=10001;
            desc.text=[dict objectForKey:@"value"];
            [desc sizeToFit];
            [_headerView addSubview:desc];
...
-(void)updateTopImageHead:(NSArray *)array
{
    for (int i=0; i<array.count; i++) {
        ...
        [imageView sd_setImageWithURL:[NSURL URLWithString:[[array objectAtIndex:i] objectForKey:@"compressPicUrl"]]placeholderImage:[UIImage imageNamed:@"defaultBannerImage.png"]] ;
...
{
    [self animationWithPicClick:[[_dataArray firstObject] objectForKey:@"clientImageBaseVos"] andTheCurrent:(int)button.tag-100 andType:0];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tag==100) {
        ...
        for (NSDictionary *dict in [[_dataArray firstObject] objectForKey:@"clientProdProductPropBaseVos"]) {
            if([[dict objectForKey:@"name"]isEqualToString:@"产品特色"])
            {
                [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[dict objectForKey:@"url"]]]];
            }
        }
    ...
    if (tag==101) {
        ...
        [cell addSubview:webView];
        for (NSDictionary *dict in [[_dataArray firstObject] objectForKey:@"clientProdProductPropBaseVos"]) {
            if([[dict objectForKey:@"name"]isEqualToString:@"出行警示及说明"])
            {
                [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[dict objectForKey:@"url"]]]];
            }
        }
    ...
    if (tag==102) {
        ...
        [cell addSubview:webView];
        for (NSDictionary *dict in [[_dataArray firstObject] objectForKey:@"clientProdProductPropBaseVos"]) {
            if([[dict objectForKey:@"name"]isEqualToString:@"预订须知"])
            {
                [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[dict objectForKey:@"url"]]]];
            }
        }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (_dataArray.count&&_headView.subviews.count==5) {
        ...
        if ([[[_dataArray firstObject] objectForKey:@"countComment"] intValue]>999) {
            commentLabel.text=[NSString stringWithFormat:@"点评(999+)"];
        }else if ([[[_dataArray firstObject] objectForKey:@"countComment"] intValue]==0)
        {
            commentLabel.text=[NSString stringWithFormat:@"点评(0)"];
        }
        else
        {
            commentLabel.text=[NSString stringWithFormat:@"点评(%@)",[[_dataArray firstObject] objectForKey:@"countComment"]];
        }
    }
...
-(void)createPicScrollView:(NSArray *)pic andTheCurrent:(int)current andType:(int)type
{
    if (!_picScrollView) {
        ...
        for (int i=0; i<pic.count; i++) {
            ...
            if (type==0) {
                [picture sd_setImageWithURL:[NSURL URLWithString:[[pic objectAtIndex:i] objectForKey:@"compressPicUrl"]] placeholderImage:nil];
            }else
            {
...
{
    [self animationWithPicClick:array andTheCurrent:count andType:type];
}
